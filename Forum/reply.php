	...
	if (isset($_POST['reply'])) {
		...
		if(time()-$last >= 10) {
			...
			if(strlen(str_replace(array("\r", "\n"), '',$reply)) >= 2 && strlen(str_replace(array("\r", "\n"), '',$reply)) <= 15000) {
				...
				$error[] = "Reply must be between 2 and 15,000 characters";
			}
		} else {
			...
			<div id="box" style="padding:10px;">
				...
				<form action="" method="POST" id="createReply">
					...
					if(isset($_GET['quote'])) {
						...
						echo '[quote][i]Quote from [url=http://tdbdnh.rocks/user/'.$authorID.'/]'.$creatorRow['username'].'[/url], '.$threadRow['date'].'[/i]
