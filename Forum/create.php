<?php 
	...
	if (isset($_POST['title']) && isset($_POST['body'])) {
		...
		if(time()-$last >= 10) {
			if (strlen(str_replace(" ","",$_POST['title'])) > 600 || strlen(str_replace(array(" ","\r", "\n"), '',$_POST['title'])) < 2) {$error[] = "Title must be between 2 and 600 characters";}
			if (strlen(str_replace(" ","",$_POST['body'])) > 25000 || strlen(str_replace(array(" ","\r", "\n"), '',$_POST['body'])) < 3) {$error[] = "Body must be between 3 and 25,000 characters";}
			if(empty($error)) {
				$time = time();
		...
		<div id="body">
			...
			<div id="box" style="padding:10px;">
				...
				<form action="" method="POST" id="createThread">
					<input type="text" name="title" id="tT" style="width: 873px;margin-top: 0px;margin-bottom: 0px;resize: none;border: 1px solid black;font-size: 16px;padding: 5px 0px 5px 5px;" placeholder="Title (max 600 characters)">
					<textarea name="body" id="tB" style="width: 869px; height: 200px; margin: 10px 0px 0px; font-size: 16px; border: 1px solid black; resize: vertical;padding: 5px 0px 5px 5px;" placeholder="Body (max 25,000 characters)"></textarea>
