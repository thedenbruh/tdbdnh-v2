...
error_reporting(0);
  ...
  'e621.net' => 'i am retаrdеd',
  'rule34.xxx' => 'i am sо much rеtаrdеd'
    );
...
<html>
  ...
  <head>
  <title> <?php echo htmlentities ( $threadRow->{'title'} ); ?> </title>
 ...
 <!-- <meta name="viewport" content="width=device-width, initial-scale=1.0"> -->
  ...
  $uid = $creatorRow->{'id'};
  $sqlfsig = "SELECT * FROM `test` WHERE `uid` = '$uid' ORDER BY `test`.`id` DESC";
  $fsigR = $conn->query($sqlfsig);
  $lolzl = $fsigR->fetch_assoc();
  $fsig = $lolzl['crap'];

    ?>
      <meta property="og:image" content="<?php echo '/assets/images/avatars/'; ?><?php echo $creatorRow->{'id'};?><?php echo ".png?c=";?><?php echo $creatorRow->{'avatar_id'}; ?>" />
      ...
      // ok
        ...
        if($page == 1) {
          echo '<div id="box" style="padding: 5px;border:0px; border-radius:0px;" class="forumPost">
          <div style="width:200px;text-align:center;float:left;">
            <div style="text-align:center" >
            <a href="/user?id='.$creatorRow->{'id'}.'" style="">'.$creatorRow->{'username'}.'<br>
            <img style="width:150px;" src="/assets/images/avatars/'.$creatorRow->{'id'}.'.png?c='.$creatorRow->{'avatar_id'}.'" /></a>
            </div>';
            
          if($creatorRow->{'power'} > '4'){
            echo '<br><span style="color: #0000BF;"><i class="fa fa-gavel" aria-hidden="true"></i> plutonium staff</span><br>';}
          ...
          echo '<span><span style="font-weight:bold;">Posts: </span>'.$userPostCount.'</span><br>';
          echo '<span><span style="font-weight:bold;">Joined: </span>'.$creatorRow->{'date'}.'</span><br>';
...
echo'<hr> '.$fsig.'<br>';} 
        ...
        <div style="margin-top:20px;float:left;">
          ...
          if($loggedIn) {
            if($power >= 5) {
              ...
              <span><a class="label" href="move?id='.$threadID.'">Move</a></span><span><span><a class="label" href="/forum/scrubfsr?id='.$creatorRow->{'id'}.'">Scrub forum signature</a></span></span>';
  ...
  $fsig = $lolzl['crap'];
          ...
          echo '<div id="box" style="padding: 5px;border:0px;border-radius:0px" class="forumPost">
            ...
            <div style="width:200px;text-align:center;float:left;">
              <div style="text-align:center" >
              <a href="/user?id='.$creatorRow->{'id'}.'">'.$creatorRow->{'username'}.'<br>
              <img style="width:150px;" src="/assets/images/avatars/'.$creatorRow->{'id'}.'.png?c='. $creatorRow->{'avatar_id'} .'" /></a>';
          ...
          echo '<br><a href="/report?type=post&id='.$threadRow['id'].'"><i style="font-size:13px;" class="fa fa-flag" style="padding-right: 5px;color:white;"></i></a>';
          echo '</span>';
          echo '</span><br>'.bbcode_to_html(nl2br(htmlentities($threadRow['body']))).'';
if(!empty($fsig)){
echo'<hr> '.$fsig.'<br>';} 
echo'</div><div style="margin-top:20px;float:left;">';
		if ($currentUserID == $threadRow['author_id']) {
              echo'<span><a class="label" href="/forum/edit?id='.$threadRow['id'].'">Edit</a></span>
              </div>';} else {echo '</div>';}
          if($power >= 5) {
            echo '<div style="margin-top:20px;float:left;">
              <span><a class="label" href="?id='.$threadID.'&scrub='.$threadRow['id'].'">Scrub reply</a></span><span><span><a class="label" href="/forum/scrubfsr?id='.$creatorRow->{'id'}.'">Scrub forum signature</a></span></span>
              </div>';
          }
          echo '</div>';
        }
