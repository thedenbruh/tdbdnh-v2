...
if($owns->num_rows > 0 && $shopRow['collectible'] == 'no') {$owns = true;} else {$owns = false;}
...
<!DOCTYPE html>
  ...
  <meta property="og:title" content="<?php echo $shopRow['name']; ?> - plutonium item" />
      ...
      echo '</h3>';
        ...
        <span style="display:inline-block; float:left; width:640px;">
          ...
          if(isset($_GET['render']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/user/rendering/shop_render?id='.$shopRow['id'].'"></iframe>';
          }
	if(isset($_GET['hatrender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/user/rendering/item/hat?id='.$shopRow['id'].'"></iframe>';
          }
	if(isset($_GET['toolrender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/user/rendering/item/tool?id='.$shopRow['id'].'"></iframe>';
          }
	if(isset($_GET['headrender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/user/rendering/item/head?id='.$shopRow['id'].'"></iframe>';
          }
if(isset($_GET['frender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/user/rendering/item/face?id='.$shopRow['id'].'"></iframe>';
          }

//trender got removed due to it is being useless

          else {
            echo '<img id="shopItem" src="/shop/thumbnails/'.$thumbnail.'.png?c='. rand() .'" style=" float:left; margin: 0px 10px 0px 10px; height: 340px; width: auto;';
            ...
            elseif($shopRow['bits'] == '0' || $shopRow['bucks'] == '0'){echo 'background-image:url(\'free_big.png\'); background-size:cover;border:0px; width:350px; height:350px;';}
            echo '">';
          }
    ...
    ?> <?php
            if ($loggedIn) {
      if ($power > 4) {
        echo '<h6>Hash: '.shopItemHash($shopRow['id']).'</h6>';
      }
    } ?>
        ...
        <span style="display:inline-block; float:right;">
          <img style="width:225px; background-color:transparent;" src="/assets/images/avatars/<?php echo $userRow['id']; ?>.png?c=<?php echo $userRow['avatar_id']; ?>">
          <h5>Created by:</h5>
      ...
      if($sellRow['active'] == 'yes') {
        ...
        <div id="subsect" style="overflow:auto;">
          ...
          <div style="width:100px;float:left;">
            <a href="/user?id='.$sellerID.'"><img style="width:70px;" src="/assets/images/avatars/'.$sellerID.'.png?c='.$sellerRow['avatar_id'].'"></a>
...
<?php } ?>
        ...
        while($commentRow = mysqli_fetch_assoc($comments)) {
              ...
              echo'<div id="subsect" style="overflow:auto;"><span style="float:left;"><a href="/user?id='.$commentUserID.'"><img style="width:55px;" src="/assets/images/avatars/'.$commentUserID.'.png?c='.rand().'"><br>'.$userData['username'].'</a></span><span"><span style="font-size:12px;">'.$commentRow['time'].'</span><br>'. htmlentities($commentRow['comment']).'</span>';
              if($loggedIn && $power >= 5) {
