...
//update as 27.11.2022 - added usernames to wall thing
  ...
  while($wallRow = $wall->fetch_assoc()) {
    if($wallRow['type'] == 'pinned') {$pin = 1;} else {$pin = 0;}
			$uID = $wallRow['owner_id'];
			$userSQL = "SELECT * FROM `beta_users` WHERE `id`='$uID'";
			$userResult = $conn->query($userSQL);
			$userRow=$userResult->fetch_assoc();
    echo '<div id="subsect" style="overflow: auto;">
    <img style="float:left; width:80px;" src="/assets/images/avatars/'.$wallRow['owner_id'].'.png"><span style="font-size:12px;">'.str_repeat('<i class="fa fa-thumb-tack"></i>',$pin).$wallRow['time'].'</span><br>'. htmlentities ( $wallRow['post'] )?><br><br>by <a href="/user/?id=<?php echo $uID?>"><?php echo $userRow['username']; ?></a>
<?php
    if($loggedIn) {
