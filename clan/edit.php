...
if(empty($error)) {
 ...
 $CN = str_replace(array('-','_','.',' '), '', $name);
    ...
    if(empty($error)) {
    $updateSQL = "UPDATE `clans` SET `name`='$name' WHERE `id`='$clanID'";
    $update = $conn->query($updateSQL); }
  }
    ...
    if($currentRank['perm_ranks'] == 'yes') {
      ...
      while($rankRow = $findRanks->fetch_assoc()) {
        ...
        if($rankPower <= $currentRank['power']) {
          $newName = str_replace(PHP_EOL, '', mysqli_real_escape_string($conn,$_POST['rank'.$rankPower]));
	 ...
	 $xss = str_replace(array('-','_','.',' '), '', $newName);
    
    if($newName != ctype_alnum($xss)) {
      $error[] = 'trying all your best on xss injecting clans? well too bad, you cannot do that';
    }
...
<!DOCTYPE html>
  <head>
    <title>Edit <?php echo $clanRow['name']; ?></title>
  </head>
  <body>
    <div id="body">
        <?php
        ...
        ?>
      <div id="box">
        <h3>Edit <?php echo $clanRow['name']; ?></h3>
        <form action="" method="POST" style="margin:10px;" enctype="multipart/form-data">
          <h4>Image:</h4>
          <img style="width:200px;height:200px;" src="/images/clans/<?php echo $clanID; ?>.png"><br>
...
<form action="" method="POST" style="margin:10px;">
    ...
    <script>
      ...
      function getRank(page, rank) {
        $("#members").load("/clan/edit_members?id=<?php echo $clanID; ?>&rank="+rank+"&page="+page);
      }
    </script>
  <?php
