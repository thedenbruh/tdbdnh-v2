						...
						if($nameExists->num_rows == 0) {
							if(isset($_POST['prefix'])) {
								if($userRow->{'bucks'} >= 0) {
									...
									if(strlen($prefix) >= 1 and strlen($prefix) <= 15) {
										if(ctype_alnum($prefix)) {
											...
											move_uploaded_file($imgTmp,"../../assets/images/clans/".$clanID.".png");
...
<!DOCTYPE html>
	<head>
		<title>New Clan - plutonium</title>
	</head>
	<body>
		<div id="body">
			<div id="box">
				...
				<form style="margin:10px;" action="" method="POST" enctype="multipart/form-data">
					...
					<span style="color:green;font-size:12px;">Creating clans are free for everyone!</span><br>
